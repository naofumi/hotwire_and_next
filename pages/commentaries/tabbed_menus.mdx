...
2. `templates/tabbed_segments_turboframes/products.ejs`に同じIDの`<section>`タグを加える
...
Next.js Pages RouterのSSRはページ全体を遷移するのには適していますが、Turbo Framesのように部分置換をする機能がありません。画面の一部分のみを置換する場合は従来のSPAと同様に、`useEffect`でステートを更新する必要があります。

一方でApp Routerは部分置換の仕組みが用意されています。[Layout](https://nextjs.org/docs/app/building-your-application/routing/pages-and-layouts#layouts)と[Parallel Routes](https://nextjs.org/docs/app/building-your-application/routing/parallel-routes)がこれに相当します。特にこのページで紹介しているタブメニューについては、Parallel Routesのドキュメントに[わざわざタブメニューの言及](https://nextjs.org/docs/app/building-your-application/routing/parallel-routes#tab-groups)もあり、この方法が奨励されているようです。
...
[Parallel Routesを使った場合のUX](http://localhost:3000/tabbed_segments_app/users)の特徴は以下の通りです
...
* 小さくて副次的な内容のタブの場合は、それは画面のメインではありませんので、URLを変化させるのは不自然です。しかしURLを変化させずにタブを切り替える方法がないようです。一方でTurbo Framesの場合は`data-turbo-action`を指定しなければ、URLをそのままにしてタブを切り替えることが可能です。
...
個人的には、たかがタブメニューを実装するために随分と大袈裟で複雑なことをするなぁと感じます。Client Componentで`useState`、`useEffect`を使ってもUX上のメリットがありませんので、わざわざParallel Routesを使う動機は弱いと感じています。
</section>
