...
Next.jsのSSRはページ全体を遷移するのには適していますが、Turbo Framesのように部分置換をする機能がありません。一方でApp Routerは部分置換の仕組みが用意されています。[Layout](https://nextjs.org/docs/app/building-your-application/routing/pages-and-layouts#layouts)と[Parallel Routes](https://nextjs.org/docs/app/building-your-application/routing/parallel-routes)がこれに相当します。

タブメニューについては、Parallel Routesのドキュメントに[わざわざタブメニューの言及](https://nextjs.org/docs/app/building-your-application/routing/parallel-routes#tab-groups)もあり、推奨されているように感じられます。

App Routerを使う場合は、コンポーネントをServer Componentで作ること([Client Componentはなるべく小さくすること](https://nextjs.org/docs/app/building-your-application/rendering/composition-patterns#moving-client-components-down-the-tree))、[なるべくサーバでデータフェッチをする](https://nextjs.org/docs/app/building-your-application/data-fetching/patterns#fetching-data-on-the-server)ことがベストプラクティスになります。しかし従来のReactと同じように`useState`、`useEffect`を使ってしまうと、ルートに近いコンポーネントをClient Componentにしなければならず、ブラウザ内でのデータフェッチも必要になってしまいます。一方でParallel RoutesはServer Componentだけでタブメニューを作る方法を提供してくれます。

そのParallel Routesを使った[デモはここから](http://localhost:3000/tabbed_segments_app/users)ご体験いただけます。またコードは[GitHub](https://github.com/naofumi/hotwire_and_next/tree/main/app/tabbed_segments_app)にあります。
...
一方でタブを切り替えてもURLを変更したくないケースもあります。特に小さいタブコンテンツの場合がそうでしょう。しかしApp RouterではURLに反映せずにタブを切り替える方法がないようです。Turbo Framesの場合は`data-turbo-action`を指定しなければ、URLに反映せずにタブを切り替えることが可能です。
...
* データフェッチはサーバ側で行いますが、動的なルート([Dynamic Rendering](https://nextjs.org/docs/app/building-your-application/rendering/server-components#dynamic-rendering))の場合は最初の`loading.js`ファイルまでしか[prefetchをしてくれません](https://nextjs.org/docs/app/api-reference/components/link#prefetch)。
...
個人的には、たかがタブメニューを実装するために随分と大袈裟で複雑なことをするなぁと感じます。Client Componentで`useState`、`useEffect`を使ってもUXが変わらないので、どれぐらい普及するかが心配です。
...
React、Next.js側と比較すると、Turbo Framesによるタブメニューは実装が簡単である割には、機能的には遜色がないことが確認できました。Reactの方はカスタムコードを書くことで細かくチューニングできるかもしれませんが、少なくとも簡単に実装できるところに限定すると、**Turbo Framesの方がむしろ高速で多機能に思えます**。
