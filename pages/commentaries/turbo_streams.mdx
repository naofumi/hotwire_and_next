...
* `products` `cart`のステートはリフトアップされているので、ステートはすべて`CartPage`コンポーネントが持っている。これがpropsで`CartIcon`, `ProductList`コンポーネントに渡される
* ステートを操作するイベントハンドラの`addToCart()`, `clearCart()`もリフトアップされるのでここに記載している。この関数もpropsで`CartIcon`, `ProductList`コンポーネントに渡される
* `useEffect`、`addToCart()`、`clearCart()`はすべて`setCart(data)`だけで、カート更新後の再レンダリングを実施している。`products`を再取得する必要はなく、`cart`の情報だけをサーバから受け取っといる
...
* CSRの特徴を活かして、`cart`情報を含めたレンダリングはブラウザで行なっている。おかげで"Add to Cart"ボタンをクリックしたとき、`products`は最初に読み込んだきりで、あとは`cart`情報だけをサーバ通信するので、ネットワーク負荷、サーバ負荷が減らせる
...
* Turbo Framesに比べて更新箇所を細かく指定できる。サーバで余計な再レンダリングをする必要がない
* これを実現するために、「何をどのように更新するか」を別途指定しなければならない。これは`<turbo-stream>`タグを使って宣言的に記述するため、ロジックはゴチャつきにくい。それでもTurbo FramesやReactの場合はそもそも更新ロジックが不要だったので、その分は複雑になっている
* 更新ロジックが単純な場合はTurbo Streamsのメリットがデメリットを上回ることも多いだろう。しかし更新ロジックが複雑な場合はTurbo FramesもしくはMorphingを使って、更新ロジックそのものを不要にした方が良いかもしれない。

なお複雑なUIでは更新ロジックが難しくなる傾向があり、[単方向データフロー](https://ja.react.dev/learn/thinking-in-react#step-2-build-a-static-version-in-react)による簡略化はReactの大きなメリットとされている。
