...
私の理解では、これはHotwireがサーバから送るHTMLと非常によく似ています。違いはClient Componentとの接続端子が用意されていることぐらいに思えます。したがってページの部分置換をするには、Turbo Framesのような仕組みが必要になってきます。

今のところ、Next.jsでTurbo Framesのような部分置換機能を提供しているのは`Layout`だけです。`Parallel routes`もありますが、これは`Layout`を拡張したもので、実質的には同じものではないかなと思っています。
