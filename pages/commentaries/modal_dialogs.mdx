...
3. [React: useEffectでモーダルを出す方法](#react-useeffect)
4. [React: Client Componentsでモーダルを出す方法](#react-client-components)
5. [React: Server Componentsでモーダルを出す方法](#react-server-components)
5. [React: Server ComponentsとLayoutでモーダルを出す方法](#react-server-components-with-layouts)
6. [各技術のまとめ](#summary)
...
ブラウザが受け取るHTMLレスポンスには空の`turbo-frame`が含まれています。Turbo Framesは既存の`turbo-frame`をこの空の`turbo-frame`zに入れ替えます。その結果、`turbo-frame`の中身は空になり、枠も無くなるのでモーダルが閉じます。ブラウザが受け取るHTMLは`turbo-frame`だけではなく、`templates/modal_no_js/index.ejs`のすべてですが、画面に反映される（差し込まれる）のは`turbo-frames`の中身(空)だけであることに注意してください。
...
1. モーダルの「枠」のHTMLをあらかじめ元のページに挿入しておきます。ただしCSSで隠して見えなくします。この中にHTML断片を差し込む目印の`turbo-frame`を入れておきます
...
**1. モーダルの「枠」だけ: `templates/modal_w_js/index.ejs`**
...
* モーダルの「枠」がページに埋め込まれています。ただしCSSで非表示になっています
* 「枠」の中に`turbo-frame id="modal"`が含まれています

**2. モーダルの内容だけを含むHTMLページ: `templates/modal_w_js/modal.ejs`**
...
* モーダルの中身だけです。枠は **1.** の方にありますので、ここには含めません。ここは純粋にモーダルの中身だけです


**3. モーダル表示のJavaScript: `templates/modal_w_js/index.ejs`**
...
* 先ほどの **1.** のモーダルの「枠」はCSSで非表示になっていました。ここのコードはその「枠」を表示させるものです
...
「モーダルの開閉」と「サーバからのデータの読み込み」を独立させたことにより、UXを大幅に改善しています。
...
Turbo Framesを使うと[カスタムJavaScriptなし](commentaries/modal_dialogs#turbo-without-custom-javascript)でもモーダルが作れます。宣伝文句としてはインパクトがあるせいか、このやり方を非常に多く見かけるように思います。しかしHotwireがStimulusとセットになっていることからも分かるように、**HotwireはむしろカスタムJavaScriptを書くことを奨励しています。JavaScriptを全く書かないのではなく、少ないJavaScriptしか書かないのがHotwireです**。そしてここで確認したように、わずか10行程度のJavaScriptでUI/UXを大きく改善できました。
...
しかしHTMLの`onclick`属性を使用しているところが問題です。`onclick`属性等の[インラインのイベントハンドラーはとても使いやすいのですが、一般には使用するべきではないとされています](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/Building_blocks/Events#inline_event_handlers_—_dont_use_these)。そこでここではHotwireに含まれる[Stimulus](https://stimulus.hotwired.dev)を使います。
...
<p className="text-gray-400 text-sm">Stimulusは[DOM MutationObserver APIを使用してDOMの変更を監視](https://stimulus.hotwired.dev/reference/lifecycle-callbacks#order-and-timing)し、そのタイミングでイベントハンドラーを接続します。このため、SPAなようにインタラクティブな要素が多いウェブページであっても、確実にかつ簡単にイベントハンドラーを接続してくれます。</p>

今回は各ポイントに軽く触れながら、Stimulusで書いたコードを紹介します。[CSSローディングアニメーションをつけたデモ](#turbo-css-loader)をベースに話します。

**1. モーダル表示のトリガー: `templates/modal_w_js/index.ejs`**
...
**2. Stimulus Controller – モーダル表示のJavaScript: `public/hotwire/javascript/labeler_controller.js`**
...
**1. モーダル表示のトリガー: pages/modal/index.tsx**
...
[useEffect](/modal)のデモでもご覧いただけるように、ちゃんとフィードバックのあるUI/UXが実現できています。コードも少なく、流れが直感的にわかります。Escapeでモーダルを閉じることも可能で、Stimulusを使用した場合と同様です。
...
* Server Componentでは、インタラクティブ要素は`a`タグと`form`タグしか使えません。ここでは`a`タグを`/modal_app`に向けました。`/modal_app`は**1.** で示した通り、モーダルを表示しませんので、実際には画面全体が再表示されているのですが、ユーザにとってはモーダルだけが消えたように見えます
...
私が理解している限りでは、RSCはページ全体をレンダリングする状況を主に想定しています。そしてRSCでインタラクティブにウェブページの部分置換をする方法は`Layout`だけです。`Parallel routes`もありますが、これは`Layout`を拡張したもので、実質的には同じものではないかなと思っています。つまりNext.jsにおいて、Turbo Framesに一番近い機能が`Layout`ではないかと思います。

これを使った場合のコードの書き味およびUI/UXを確認します。
...
* React Server Componentsを使ったモーダルの作り方がNext.jsの公式サイトに紹介されています。しかしコードのわかりやすさとUI/UXを総合的に加味すると、現時点ではお勧めするのは難しいでしょう。
