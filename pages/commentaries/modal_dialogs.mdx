...
* モーダルの中身だけです。枠は**1.**の方にありますので、不要です
...
* **1.**のモーダルの「枠」はCSSで非表示になっていました。ここのコードはそれを表示させるものです
* `script`タグで囲んでインラインで書いています。通常であればStimulusを使うのですが、ここでは説明するためのわかりやすさを優先しました。
