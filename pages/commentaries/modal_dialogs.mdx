...
1. [Turbo Frames: カスタムJavaScriptなしでモーダルを出す方法](/commentaries/modal_dialogs#turbo-without-custom-javascript)
2. [Turbo Frames: カスタムJavaScriptを追加してモーダルのUI/UXを改善する方法](/commentaries/modal_dialogs#turbo-with-custom-javascript)
    1. JavaScriptをインラインで書いた場合
    2. Stimulusを使った場合
    3. さらにCSSでローディングアニメーションを追加
...
<H2WithHash id="turbo-with-custom-javascript">Turbo Frames: JavaScriptあり</H2WithHash>

上記の[Turbo Frames: カスタムJavaScriptなし](http://localhost:3000/commentaries/modal_dialogs#turbo-without-custom-javascript)の問題点を解消し、ネットワークが遅い場合でも十分なUI/UXを提供するアプローチになります。上記ではモーダルの「枠」をサーバから送ることでモーダルを開くステップを省略していました。しかしここではちゃんとJavaScriptを使って、**サーバレスポンスの遅延の有無に関わらず、リクエストを投げた瞬間にモーダルを開きます**。これがユーザに対するフィードバックとなり、UI/UXの改善に繋がります。
...
2. モーダルの表示をトリガーするJavaScriptの設置。Turboがサーバにリクエストを投げるのは変わらないのですが、これとは独立にモーダルを表示するJavaScriptを用意し、リンクをクリックしたときにこれも合わせて実行されるようにします。
...
JavaScriptなしの場合と比べると、モーダルの「枠」がないのが特徴です。モーダルの中身だけを置いておきます。
