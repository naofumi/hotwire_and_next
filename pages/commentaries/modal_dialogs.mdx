...
* Client Componentのみを使った場合はSEOの問題があります。モーダルの中身も含めて検索エンジンにヒットさせたい場合は(少なくとも部分的に)Server Componentを使う方が良いでしょう
...
### コード ===code-react-server-components-with-layouts===

[コードはGitHubに掲載](https://github.com/naofumi/hotwire_and_next/tree/main/app/modal_app_layout)しています。また[デモはこちら](/modal_app_layout)でご確認いただけます。
...
### UI/UX ===ui-react-server-components-with-layouts===
...
### まとめ ===summary-react-server-components-with-layouts===
...
* サーバのレスポンスが遅いケースでモーダルを閉じる際は、簡易方と同じUI/UXの問題が出てきます

コードが複雑なのがこのアプローチの欠点だと思います。[Parallel routes](https://nextjs.org/docs/app/building-your-application/routing/parallel-routes)や[Intercepting routes](https://nextjs.org/docs/app/building-your-application/routing/intercepting-routes)を使ってもベースの仕組みは同じなので特に改善するとは予想していません。したがって私としては現時点でおすすめはできません。
