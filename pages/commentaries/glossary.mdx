...
* Reactアプリであり、CSRであっても、MPAというサイトはたくさんあります。例えば[Apple Store](https://www.apple.com/jp/shop/buy-iphone/iphone-15)はReactで作成されていて、JavaScriptをオフにすると正しく表示されません。つまりCSRが必須になっています。一方で画面遷移するときにメモリ上のJavaScriptとCSSはすべて削除され、古いページが完全に消されてから新しいページに切り替わります。これはブラウザのローダーアニメーションおよびネットワークタブから確認できます。多くのJavaScriptを使ってリアクティブにしたいページだけはReactを使い、その他はReactなしで書くケースはこの使い方をします。
