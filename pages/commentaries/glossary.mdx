...
CSR, SSR, SPA, MPAといった用語はモダンフロントエンドのメリット・デメリットを理解する上で必須の用語です。一方で人によって意味がまちまちであり、議論の土台とするのに苦労します。ここではこれらの用語に明確な定義を与え、技術比較の助けにしたいと思います。
...
* React Server ComponentsはRSC payloadをサーバからブラウザに送ります。これはHTMLにかなり近いもので、HTMLの構造がそのまま記載されています。したがってSSRとして捉えます
...
一般的な用法により近いのは4.のステージだと思いますが、Reactの世界では主に2.のステージを指しているように思います。そこで**本サイトでも「レンダリング」は2.の意味で使用します**。広義にはブラウザDOMに反映する直前の状態がレンダリングだと考えており、React Server ComponentsのRSC Payloadの出力もレンダリングと考えています。
