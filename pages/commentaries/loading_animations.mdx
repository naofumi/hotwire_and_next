...
上記のNewspicksのウェブサイトはNext.jsのSSRで動いています。2ページ目以降はAjax/fetchを使って遷移します。だからローディングアニメーションが表示されません。
...
NewsPicksはサーバのレスポンスが遅いサイトです。ローディングアニメーションの導入はぜひ検討するべきでしょう。それが難しい場合は、Next.jsの`Link`タグによる[SPA的画面遷移](/loading_animations#loading-animations-with-ssr)を諦め、`a`タグを使用して[ブラウザネイティブなMPA画面遷移](/commentaries/loading_animations#loading-animations-with-native)に戻した方がむしろUXは改善するでしょう。
...
Turbo Framesがロード中の時は`turbo-frame`タグに[`busy`の属性がつけられます](https://turbo.hotwired.dev/reference/frames#html-attributes)。同様に[`aria-busy`もつきます](https://turbo.hotwired.dev/reference/attributes#automatically-added-attributes)。したがってCSSを使うことで、`turbo-frame`周辺に限定したローディングアニメーションがつけられます。
...
* ページの部分置換(Turbo Frames)については、HotwireもNext.jsもデフォルトではローディングアニメーションを用意していません。しかしHotwireはCSSを使えば良いですし、Next.jsもApp RouterであればSuspenseで簡単にローディングアニメーションを実装できます。またPages Router SSRではそもそもページの部分置換はできず、`useEffect`と条件付きレンダーを使用するしかありませんので、SSR化のためにUXが落ちることも起こりません。
* なおHotwireの場合はprefetchを使って、リンクをクリックする前からフライングをしてリンク先のページを読み込むことができます。Turbo Driveの時もTurbo Frameの時もprefetchは有効です。Next.jsでは、動的ページの場合、これができません。その分だけHotwireの反応は速くなり、より優れたUXになります。
