...
* **Turbo Drive:**[Turbo Drive](https://turbo.hotwired.dev/handbook/drive)は`body`タグの中身を丸々置換する技術です。Next.jsの[router](https://nextjs.org/docs/pages/building-your-application/routing)、あるいは[React Router](https://reactrouter.com/)に相当し、ページ全体をSPA的に置換・遷移する仕組みです。`Link`タグのような特別なものは使わず、`a`タグがすべて自動的にTurbo Driveを使うようになります。
...
Reactの世界ではuseStateに始まり、Reduxに至るまで、ステート管理が非常に大きなトピックになっています。ここではHotwireにおけるステートの一般的な考え方について紹介します。

* **Reactは単方向データフロー(one-way data flow):** Reactでは単方向データフローが非常に重要な概念です。すべてはステートから始まり、親コンポーネントから子コンポーネントにステートを渡しながら画面がレンダリングされます。ステートは各コンポーネントが`useState`で保持したり、あるいはグローバルに`useContext`やReduxなどを使って保持されます。ユーザの操作等によって画面の複数箇所が同時に更新され、さらに操作をする箇所が多く、複雑なアプリにおいてはこれは非常に有効なコンセプトです
* **Turboはサーバにステート管理を任せます:** HotwireのTurboはサーバにステートを持たせ、同じものはブラウザでは持たないという考え方です。HTMLはサーバでレンダリングされますので、ブラウザに送られてくる必要がありません。またHTML `form`によってサーバにデータを送信する場合も、`form`自身がネイティブに保有するステート管理に任せます。Reactのように`form`の状態をステートに持たせ、それをJSONに変換してサーバに送ることをしません。`form`のネイティブな動作に任せ、`application/x-www-form-urlencoded`形式でサーバに送ります。その一方で積極的にキャッシュを行い、ネットワーク通信による遅延を最小化する仕組みを各種用意しています。
...
このようにHotwireのステート管理の考え方は、サーバが持つべきものはTurbo経由でサーバに持たせ、ブラウザが持つべきものはある程度複雑なものまでStimulusで処理するというものです。Stimulusの担当範囲は"sprinkles of JavaScript"(「軽く撒いたJavaScript」)と呼ばれます。それ以上のものが必要な場合は別の技術に委ねます。


## 本サイトの構成 ===structure-of-this-site===

* 本サイトのコードはすべて[GitHub](https://github.com/naofumi/hotwire_and_next)に公開しています。また[Vercel](https://hotwire-and-next.vercel.app)でデモ用にデプロイしています。コードをGitHubで確認しながら、UXをデモで確認すると理解が深まるのではないかと思います。
...
* **データベースは使用しない:** デモサイトの運用を考えて、実際にはデータベースを使っていません。システムのメモリやCookie等に持たせています。
