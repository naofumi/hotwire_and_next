...
* **Reactは単方向データフロー(one-way data flow):** Reactでは単方向データフローが非常に重要な概念です。全てはステートから始まり、親コンポーネントから子コンポーネントにステートを渡しながら画面がレンダリングされます。ステートは各コンポーネントが`useState`で保持したり、あるいはグローバルに`useContext`やReduxなどを使って保持されます。ユーザの操作等によって画面の複数箇所が同時に更新され、さらに操作をする箇所が多く、複雑なアプリにおいてはこれは非常に有効なコンセプトです
